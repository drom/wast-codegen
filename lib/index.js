...
const exprGen = {
  ...
  assert_return_nan: function (node) { // ['invoke' ],
    return `(assert_return_nan ${exprGen[node.invoke.kind](node.invoke)})`
  },
  ...
  call_import: function (node) { // ['id', 'expr' ],
    let res = `(call_import ${exprGen[node.id.kind](node.id)}`
  ...
  call_indirect: function (node) { // ['id', 'expr' ],
    let res = `(call_indirect ${exprGen[node.id.kind](node.id)}`
    res += node.expr.map((e) => exprGen[e.kind](e)).join(' ')
    res += ')'
    return res
  },
